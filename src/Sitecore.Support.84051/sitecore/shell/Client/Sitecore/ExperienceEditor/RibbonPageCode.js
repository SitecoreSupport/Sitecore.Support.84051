  ...
  function (Sitecore) {
    var RibbonPageCode = Sitecore.Definitions.App.extend({
      ...
      initializeNotifications: function (context) {
        ...
        Sitecore.ExperienceEditor.instance.NotificationBar.viewModel.$el.on("click", "button.close", function (e) {
          ...
          for (var i = 0; i < notifications.length; i++) {
            ...
            if (notificationElement
              ...
              for (var j = 0; j < notification.Options.length; j++) {
                $(notificationElement).append(context.getNotificationOption(notification.Options[j].Title, notification.Options[j].Command));
              }
            }
          }
        }).execute(context);
      ...
      getNotificationOption: function (notificationTitle, notificationCommand, notificationId) {
        ...
        if (notificationCommand) {
          notification += "onclick=\"javascript: return window.parent.scForm.postEvent(this, event, '" + notificationCommand + "')\"";
        }
      ...
      processItemLocking: function (context) {
        ...
        var notificationTitle = Sitecore.ExperienceEditor.TranslationsUtils.translateText(Sitecore.ExperienceEditor.TranslationsUtils.keys.You_must_lock_this_item_before_you_can_edit_it);
        var notification = this.showNotification("warning", notificationTitle, true);
        var notificationOption = $(this.getNotificationOption(Sitecore.ExperienceEditor.TranslationsUtils.translateText(Sitecore.ExperienceEditor.TranslationsUtils.keys.Lock_and_edit)));
