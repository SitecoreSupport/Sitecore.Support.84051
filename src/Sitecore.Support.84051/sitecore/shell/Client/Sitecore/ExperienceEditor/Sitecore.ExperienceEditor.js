...
Sitecore.ExperienceEditor = {
  ...
  navigateToItem: function (itemId) {
    ...
    var virtualFolder = this.instance.currentContext.virtualFolder;
...
Sitecore.ExperienceEditor.Common = {
  ...
  removeNotificationMessage: function (messageText) {
    Sitecore.ExperienceEditor.instance.NotificationBar.removeMessage(function (message) {
...
Sitecore.ExperienceEditor.CommandsUtil = {
  ...
  getCommands: function(context) {
    ...
    $.each(context.app, function () {
      ...
      commands.push({
        initiator: this,
        command: Sitecore.Speak.Commands[this.get("command")]
      });
    });
  ...
  checkCommandsCanExecute: function(context) {
